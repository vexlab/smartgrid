	...
	public AssetDTO get(String assetId) {
		...
		if (a != null) {
			...
			return new AssetDTO.Builder()
					...
					.devices(asset.getDevices().stream().map(Device::getId).map(UUID::toString).collect(Collectors.toList()))
					.type(asset.getType().stream().map(AssetType::getId).map(UUID::toString).collect(Collectors.toList()))
