...
public class CustomerServiceImpl implements CustomerService {
	...
	public GenericDataDTO create(CustomerDTO customerDTO) {
		...
		if (customerDTO.getId() != null) {
			Optional<Customer> customer = customerRepository.findById(UUID.fromString(customerDTO.getId()));
		...
		List<String> assetList = customerDTO.getAssets() != null ? customerDTO.getAssets() : new ArrayList<String>();
		...
		c.setAssets(assets);
		List<String> dashboardList = customerDTO.getDashboards() != null ? customerDTO.getDashboards()
	...
	public GenericDataDTO delete(String customerId) {
		...
		return new GenericDataDTO.Builder().id(customerId).display("Generic error.").error(true).build();
	}
	...
	public List<GenericDataDTO> getAll() {
		...
		for (Tuple tuple : resultList) {
			...
			dtos.add(new GenericDataDTO.Builder()
					.id(id)
					.display(title)
					.additionalData(data)
					.build());
	...
	public CustomerDTO get(String customerId) {
		...
		if (c != null) {
			...
			return new CustomerDTO.Builder()
					...
					.assets(customer.getAssets().stream().map(Asset::getId).map(UUID::toString).collect(Collectors.toList()))
					.dashboards(customer.getDashboards().stream().map(Dashboard::getId).map(UUID::toString).collect(Collectors.toList()))
